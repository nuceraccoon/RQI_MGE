...
\begin{center}
  ...
  {\large \bf Eigenvalue Solvers for Modeling Nuclear Reactors on Leadership Class Machines}
...
The real motivation of this work was to add RQI, which should converge in fewer iterations and less time than PI, and possibly Arnoldi, for large and challenging problems. RQI creates shifted systems that would not be tractable without the MG Krylov solver. It also creates ill-conditioned matrices that cannot converge without the MGE preconditioner. RQI also often needs help starting in the right direction, and performing one initial PI was shown to improve performance. Using these methods, RQI converged in fewer iterations and in less time than both PI and Arnoldi for large problems on large core counts. 
