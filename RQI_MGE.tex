...
Denovo has also implemented an Arnoldi Krylov subspace solver available for solving eigenvalue problems \cite{Davidson2013}. Like RQI, it can be parallelized over energy by using energy sets. Depending on how the energy decomposition is requested, two "modes" can be used: a Gauss-Siedel/Krylov combination or a pure Krylov solver. When upscatter partitioning is enabled, only the upscatter groups are decomposed into sets. The downscatter groups are replicated and solved using Gauss-Siedel while the upscatter groups are solved with a Krylov solver. For full partitioning, all groups are decomposed into sets and only the Krylov solver is used. Upscatter partitioning has been shown to initially perform better than full partitioning, but does not scale well over many energy sets \cite{Davidson2014}.
