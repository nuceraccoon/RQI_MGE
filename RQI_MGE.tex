...
We needed to study multiset scaling for a problem of this size with updated method, and we wanted to compare performance to Arnoldi. Thus, the BW1484 problem was solved with RQI with and without preconditioning and compared to Arnoldi iteration with both upscatter and regular partitioning. 

We started by performing a smaller study on a local cluster, Remus. This problem used 2 $\times$ 2 spatial cells/pin with 60 $\times$ 60 pins for a total of 120 $\times$ 120 $\times$ 44 mesh elements (633,600 cells). Step characteristic was used giving 1 unknown/cell; $P_0$ scattering expansion; an $S_{48}$ angular quadrature; and 56 energy groups - which gave approximately 425 million unknowns. In the preconditioned cases, an $S_1$ quadrature was used in the multilevel preconditioner with 2 v-cycles and a relax count of 3.

On the cluster, the study was performed with 128, 256, and 512 cores and 1, 2, and 4 energy sets, respectively. Figure \ref{fig:remus} shows the results.
\begin{figure}
\label{fig:remus}
...
The cluster results showed favorable scaling with both preconditioned and unpreconditioned RQI. Preconditioned RQI was faster than unpreconditioned. Both were faster than Arnoldi with full partitioning (which scaled well, but was the slowest). Arnoldi with upscatter partitioning performed the best at low core count, but shows poor scaling performance after two energy sets and ended in 3rd place with respect to time. 

To see how these trends would hold on larger problems and with more cores, we also performed this study on Titan. Here the 1-set case was run with 3,136 cores (56 $x$-blocks and 56 $y$-blocks). A parallel-efficiency modeling tool developed at Oak Ridge was used to choose the optimal $z$-blocks for each method, which in the case of RQI with MGE preconditioning was determined to be 22. For the other three methods, 11 $z$-blocks were determined to be optimal. We then scaled up to 2, 4, and 8 sets and 6,272, 12,544, and 25,088 cores, respectively. The upscatter and k-tolerances were set to $1 \times 10^{-5}$. We used 8 mesh cells/pin for a total of 336x336x154 mesh elements (17,385,984 cells). Based on previous studies \cite{Slaybaugh2015}, a weight of 1 with 3 v-cycles and 2 relaxations was chosen for the MGE preconditioner. 
and \ref{fig:titan} 
\begin{figure}
\label{fig:titan}
...
All of the same behaviors were observed as the smaller case. MORE WORDS HERE.
