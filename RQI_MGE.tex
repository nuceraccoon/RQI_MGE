...
A common way to solve $k$-eigenvalue problems is with Power iteration (PI). This method is attractive because it only requires matrix-vector products and two vectors of storage space. PI uses the form of the problem seen in Eq. \eqref{eq:EnergyDepEval} and then iterates as shown in Eq.\ \eqref{eq:PowerIteration}, where $i$ is the iteration index. This converts the generalized form of the eigenvalue problem seen in Eq.\ \eqref{eq:OperatorEvalForm} to the ordinary form. In the generalized form, the eigenvector-value pair is $(\phi, \frac{1}{k})$, and in the ordinary form it is $(\phi, k)$. 
...
Paige et al.\ \cite{Paige2006} demonstrate that GMRES is backwards stable when finding $x$ in $\ve{A}x = b$ for a ``sufficiently nonsingular $\ve{A}$'', and define associated criteria. For ill-conditioned systems, then, Krylov methods may not be backwards stable and will tend to converge very slowly. Many researchers have found that Krylov methods must be preconditioned to be able to get good results in practice \cite{Benzi2002}, \cite{Saad1986}, \cite{Trefethen1997} , \cite{Paige2006}. The past work section below demonstrates that the RQI needs preconditioning for convergence in problems of interest.
...
Recall that $\ve{A} = \ve{I} - \ve{TMS}$. To implement multigrid in energy as a right preconditioner in Denovo, $\ve{G}^{-1}$ represents the appication of the preconditioner, $y$ is defined as $\ve{G}\phi$, and the problem is broken into two steps: 
%
\begin{enumerate}
...
(Physor Paper)
\begin{enumerate}
 \item RQI works as well as PI from a time and iteration count perspective for very simple problems. (simple eigenvalue test problem)
...
The first set of fixed-source tests provided a basis for chosing preconditioning parameters. These two sets of results showed that increasing $r$ and/or $v$ decreased Krylov iteration count. They also indicated that using a small amount of weight can be beneficial, but a large amount is not. As a result, the default parameter recommendations are $r2v2w1$. 

\begin{enumerate}
 \item demonstrated that using a reduced angle set inside the preconditioner is very valuable with FeC cube
 \item FeC problem showed very good energy scaling behavior when full grid-depth is chosen 
 \item FeC problem also showed that using a reduced grid depth is just as beneficial as full grid depth (likely makes energy scaling less impressive)
