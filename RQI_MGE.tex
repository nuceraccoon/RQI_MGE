...
The RQI method added to Denovo uses the MG Krylov solver, which is designed to handle dense scattering matrices effectively--overcoming the burden of using of a dense $\ve{\tilde{S}}$. In addition, RQI can be decomposed in energy and take advantage of the scaling properties of the multigroup solver. Using RQI in combination with a Krylov method, however, raises some concerns about whether or not it can converge. 
...
\end{equation}
where $k$ is the eigenvalue and $\Gamma$ is the eigenvector. We used the energy-dependent implementation in all of our calcualtions. Like RQI, the Arnoldi solver can be parallelized over energy by using energy sets enabled by the MG Krylov solver.
...
In the first one or few RQI iterations, the eigenvector guess, and therefore the computed RQ, may not be be very close to the dominant eigenmode. If, however, the RQ is close to another eigenvalue, the system may converge to a non-dominant eigenvalue/vector pair without warning. Or, the RQ may never become a good guess or could bounce between eigenvalues that are close to one another. If this happens RQI may not converge at all, which we have witnessed in past work. 

One approach to alleviate this issue is to begin the solution process with one or a few Power Iterations before beginning Rayleigh Quotient Iteration. This will increase the likelihood that the first shift RQI uses is close to the goal. The idea is to help ensure that RQI will converge, and that RQI will converge to the correct eigenpair. 

In our work we have implemented this idea by using a zero shift in the first RQI iteration. This is functionally performing PI once before the standard RQI method is used. Some of the results we generated, discussed below, were \textit{without} the PI iterations to start. We found, however, that implementing this ``PI-start'' was frequently needed. The results that used one Power Iteration at the outset will be indicated.
...
These cases confirmed that the RQI and MGE solvers could work together, showed that the correct solution is obtained with the unshifted preconditioner as long as the problem converges, and that Krylov iteration count is reduced when RQI is used with MGE. Both tests exhibited the same patterns for weight, number of relaxations per level, and number of V-cycles as the previously-published MGE results--demonstrating that RQI did not change the parameter behavior of the preconditioner. These tests also confirmed that the problems associated with shifting the operator persist when used inside of Richardson iteration, and that the unshifted version should be used to ensure stability.
...
We also considered what would happen with a reduced quadrature set inside the preconditioner. As can be seen in the ``RQI w/ $S_{2}$'' case, this had a big time saving impact: reducing the solver time from 13.36 to 3.19 hours, or 76\%, without affecting the solution. All further PWR 900 calculations used $S_{2}$ in the preconditioner.
...
To more conclusively resolve the comparison, we did another study, this time on Titan. We used a V-cycle depth of 2, a reduced quadrature in the MGE preconditioner of $S_2$, and, unless otherwise noted, tolerances of $1 \times 10^{-3}$. This time we broke up over 112 $\times$ 112 $\times$ 10 partitions (12,544 blocks).
...
To investigate how the full system of solvers performs for a real problem, a strong scaling study using RQI with MGE using $w1r2v2$ and 1, 4, 11, and 22 sets was done. 
...
For this problem, every eigenvalue iteration failed to converge the eigenvector, reaching 100 Krylov iterations without a residual under $1 \times 10^{-8}$. The $k$ at the end of 4 eigenvalue iterations was not close to the reference value. 

After the RQI solver was updated to first perform one PI iteration to get on track, this problem converged without trouble: the eigenvector converged in under 65 Krylov iterations inside of each RQI iteration, and 6 RQI iterations were required to converge $k$. 

This 2D BW1484 problem illustrated that the PI shift can make a big difference in terms of ability to converge problems that RQI is struggling with. To investigate the performance of the updated RQI method and compare it to Arnoldi, a much more competitive comparison than to PI, we looked at a larger version of this problem.
...
We started by performing a smaller study on a local cluster, Remus. This problem used 2 $\times$ 2 spatial cells/pin with 60 $\times$ 60 pins for a total of 120 $\times$ 120 $\times$ 44 mesh elements (633,600 cells). We used a step characteristic spatial method, giving 1 unknown/cell; a $P_0$ scattering expansion; an $S_{48}$ angular quadrature; and 56 energy groups--which gave approximately 425 million unknowns. In the preconditioned cases, an $S_1$ quadrature was used in the multilevel preconditioner with, a weight of 1, 2 v-cycles, and a relax count of 3.

On the Remus cluster, the study was performed with 128, 256, and 512 cores and 1, 2, and 4 energy sets, respectively. Figure \ref{fig:remus} shows the results.
\begin{figure}
\label{fig:remus}
...
\end{figure}
%
We can see favorable scaling with both preconditioned and unpreconditioned RQI. Preconditioned RQI was faster than unpreconditioned. Both were faster than Arnoldi with full partitioning (which scaled just as well, but was the slowest). Arnoldi with upscatter partitioning performed the best at low core count, but shows poor scaling performance after two energy sets and ended in 3rd place with respect to time at higher core counts. 

To see how these trends would hold on larger problems and with more cores, we also performed this study on Titan. Here the 1-set case was run with 3,136 cores (56 $x$-blocks and 56 $y$-blocks). A parallel-efficiency modeling tool developed at Oak Ridge was used to choose the optimal $z$-blocks for each method, which in the case of RQI with MGE preconditioning was determined to be 22. For the other three methods, 11 $z$-blocks were determined to be optimal. We then scaled up to 2, 4, and 8 sets using 6,272, 12,544, and 25,088 cores, respectively. The upscatter and k-tolerances were $1 \times 10^{-5}$. We used 8 mesh cells/pin for a total of 336 $\times$ 336 $\times$ 154 mesh elements (17,385,984 cells). Based on previous studies \cite{Slaybaugh2015}, a weight of 1 with 3 v-cycles and 2 relaxations was chosen for the MGE preconditioner. 
The results can be seen in Figure~\ref{fig:titan}.
\begin{figure}
\label{fig:titan}
...
The same trends were seen on Titan as on Remus. Arnoldi with full partitioning scaled well, but was the slowest. RQI with and without preconditioning scaled similarly, with the preconditioned version being faster. Arnoldi with upscatter partitioning started out as the fastest, but scales the least well. Thus, at 4 and 8 energy sets it ended up much slower. 

The Arnoldi behavior makes sense given the way the partitioning works. With full partitioning, all of the groups are divided into energy sets solved with a Krylov solver. Thus, adding more sets increases the parallelization in energy. With only the upscattering partitioned the downscatter only groups, which are half of the groups (4) in this case, are always solved with GS. For this problem, using more than 4 does not result in additional parallelization in energy and we see the scaling stagnate. 

From this behavior we can conclude that problems that are small enough to solve without full energy parallelization, Arnoldo with upscatter partitioning should be used. For problems that are large enough to need more energy parallelization, RQI with the MGE preconditioner should be selected. 
