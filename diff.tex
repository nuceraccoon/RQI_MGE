...
%DIF DEL RQI_MGE.tex                  Mon Nov 27 15:21:00 2017
%DIF ADD RQI_reviewer_addressed.tex   Mon Nov 27 15:17:39 2017
...
\title{Eigenvalue Solvers for Modeling Nuclear Reactors on Leadership Class Machines} %title of paper

\author{
\vspace{20mm}
\\R.N.\ Slaybaugh,$^{\text{a},\ast}$ M.\ Ramirez-Zweiger,$^{\text{a}}$ \\Tara Pandya,$^\text{b}$ Steven Hamilton,$^\text{b}$ and T.M. Evans$^\text{b}$\\[4pt] %list of authors, with corresponding author marked by asterisk
\textit{$^a$University of California, Berkeley, Nuclear Engineering Department}\\[-10pt]       %affiliations of authors
...
\DIFdelbegin \DIFdel{As a result}\DIFdelend \DIFaddbegin \DIFadd{Each additional Krylov iteration increases the subspace size by one multi-group-sized vector and the cost of an iteration application correspondingly increases. 
Therefore}\DIFaddend , any reduction in iteration count will have a significant benefit in terms of memory and cost per iteration. 
...
%However, we implemented the option to use either for investigation purposes. The default behavior when doing RQI is to use the regular, unshifted operator in the preconditioner, and the use of the shifted operator can be turned on through an input option.
...
When using multiple energy sets, \DIFdelbegin \DIFdel{cross-set communication is prohibited by having each set do }\DIFdelend \DIFaddbegin \DIFadd{each energy set does }\DIFaddend its own ``mini'' V-cycle. 
Each set restricts, prolongs, and relaxes on only its own groups. 
\DIFaddbegin \DIFadd{This means that cross-set communication is not needed, which is advantageous for scaling in energy.
}\DIFaddend In the case of an unequal number of groups per set, all sets \DIFdelbegin \DIFdel{are forced to have the same grid depth}\DIFdelend \DIFaddbegin \DIFadd{use the shallowest grid depth, which is determined by the set with the fewest groups, }\DIFaddend to enforce energy load balancing between sets. Thus, each set restricts to one or two groups, giving approximately $num\_sets$ total groups across all sets at the coarsest level. The number of grids needed is determined by the set with the minimum number of groups, since it will be the first to reach a grid with one group:%. This modifies Equation~\eqref{eq:NumGrids} to be
\begin{align}
...
%PI    & 1   & 10,200   &  &  &       &  &  \\
RQI & 4   & 40,800   & 1.269 \DIFaddbeginFL \DIFaddFL{$\pm$ 1.12e-3 }\DIFaddendFL & 76   &  6               & 802.60  \\
%RQI w/ $S_{2}$ & 4   & 40,800   & 1.269 & 79   &  6               & 192.48    \\
PI    & 4   & 40,800   & 1.270 \DIFaddbeginFL \DIFaddFL{$\pm$ 6.68e-2 }\DIFaddendFL & 101 & 10$^{+}$ & 1440.12  \\
RQI & 11 & 112,200 & 1.269 \DIFaddbeginFL \DIFaddFL{$\pm$ 1.12e-3 }\DIFaddendFL & 76   & 6                & 331.43    \\
PI    & 11 & 112,200 & 1.270 \DIFaddbeginFL \DIFaddFL{$\pm$ 5.09e-2 }\DIFaddendFL & 111 & 11$^{+}$ & 480.63     \\
...
\end{table}  
%DIF >  tol = 1e-3, uptol = 1e-4, ktol = 1e-3
...
The results using 11 energy sets are given in Table~\ref{tab:PWR all} and illustrate several things. One \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{result }\DIFaddend is that the MGE preconditioner does not
help PI; when we add it, the calculation slows down enough that it did not
finish within \DIFdelbegin \DIFdel{the wall time limit here either (it does finish without it).  Another result is that RQI is too slow without enough preconditioning. With $w1r1v1$, the calculation did not complete in time and with $w1r2v2$ it did. The final, and arguably most important , }\DIFdelend \DIFaddbegin \DIFadd{available wall time limits, although convergence does occur
eventually.  The most important }\DIFaddend result is that preconditioned RQI can be much
...
\label{tab:PWR all}
  \begin{center}
    \begin{tabular}{| c | c | c | c | c | c | c |}
      ...
      %%
      %DIF <  RQI & none   & info &     &                     & \\
      PI   & none     & 149 & 5602 & 1.276 $\pm$ \DIFdelbeginFL \DIFdelFL{4.34e-6 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.85e-3 }\DIFaddendFL & 612.2 \\
      %DIF < PI  & w1r1v1 & info &     &                     & \\
      PI   & w1r2v2 & 86   & 946   & 1.275 $\pm$ 1.43\DIFdelbeginFL \DIFdelFL{e-5 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{e-3 }\DIFaddendFL & 720$^+$ \\
      \DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD <       %%%
\DIFdelendFL \DIFdelFL{RQI } & \DIFdelbeginFL \DIFdelFL{w1r1v1 } &%DIFDELCMD < & %%%
\DIFdelFL{3   } & %DIFDELCMD < & %%%
\DIFdelFL{45   } & %DIFDELCMD < & %%%
\DIFdelFL{1.252 $\pm$ 1.13e-2 } & %DIFDELCMD < & %%%
\DIFdelFL{120$^+$ }%DIFDELCMD < \\
%DIFDELCMD <       %%%
\\
RQI &%DIFDELCMD < & %%%
\DIFdelendFL w1r2v2 & 5     & 70     & 1.268 $\pm$ \DIFdelbeginFL \DIFdelFL{9.75e-4 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.24e-3 }\DIFaddendFL & 54.8 \\
      %DIF < \hline
      %DIF < PI  & none   & 24  & 901  & 1.272 $\pm$ 1.01e-4 & 92.1$^\dagger$ \\
      %DIF < PI  & w1r1v1 & info &     &                     & \#$^\dagger$\\
      %DIF < PI  & w1r2v2 & 28  & 312  & 1.273 $\pm$ 8.24e-5 & 250.9$^\dagger$ \\
      %DIF < PI  & w1r3v3 & 11  & 111   & 1.270 $\pm$ 5.09 $\times$ 10$^{-2}$ & 480$^{*,\dag}$ \\
      \hline
    \end{tabular}\\
...
%%---
%RQI is an old method that is an adaptation of shifted inverse iteration. Because the RQ provides an optimal shift, it is expected that using RQI in Denovo will converge in fewer iterations than PI for loosely coupled problems, provided that the eigenvector is converged. 
%
